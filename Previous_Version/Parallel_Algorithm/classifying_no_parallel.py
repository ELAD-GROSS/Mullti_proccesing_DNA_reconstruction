...
def run_section_algorithm(section_reads_lst: list, paddings_positions, section_len, read_size,
    ...
    if candidate_results is None or len(candidate_results) != 1:
        ...
        complete_sections_dict[section] = list(candidate_results)[0]


def run_iterative_algorithm(reads_lst, paddings_by_sections, read_size, real_edge_length,
...
def final_algorithm(sections_num, letters_amount, classify, real_edge_len, frequency, strand_len, g_freq, read_size,
    ...
    print(f"declassify time: {time.time() - time_declassify}")
    ...
    complete_sections = run_iterative_algorithm(reads_by_sections, paddings_by_sections, read_size,
