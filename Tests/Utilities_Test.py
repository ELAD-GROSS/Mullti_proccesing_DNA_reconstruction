...
def add_extra(strand, unused_letters, read_size, unused_arr, reads_starts, reads, window_start):
...
def generate_reads(strand, strand_length, read_size):
    ...
    add_extra(strand, unused_letters, read_size, unused_arr, reads_starts, reads, 0)
    add_extra(strand, unused_letters, read_size, unused_arr, reads_starts, reads, strand_length - read_size)

    while unused_letters[0] > 0:
        window_start = random.randint(0, strand_length - read_size)
        if reads_starts[window_start]:
            continue
        add_extra(strand, unused_letters, read_size, unused_arr, reads_starts, reads, window_start)
